...
if (isset($_GET['cari'])) {
    ...
    $query = "SELECT b.*, k.nama_kategori, s.nama_supplier FROM barang b 
              LEFT JOIN kategori k ON b.id_kategori = k.id 
              LEFT JOIN supplier s ON b.id_supplier = s.id 
              WHERE b.nama_barang LIKE '%$search%' OR b.kode_barang LIKE '%$search%' OR k.nama_kategori LIKE '%$search%' OR s.nama_supplier LIKE '%$search%' 
              ORDER BY b.id DESC";
} else {
    $query = "SELECT b.*, k.nama_kategori, s.nama_supplier FROM barang b 
              LEFT JOIN kategori k ON b.id_kategori = k.id 
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Poppins', sans-serif; }
        ...
        .table-wrapper { max-height: 450px; overflow-y: auto; background: white; border-radius: 12px; border: 1px solid #e2e8f0; }
...
<body>
    ...
    <div class="main-content">
        <div class="header"><h2>Daftar Informasi Stok & Supplier (Staf)</h2></div>
        <div class="search-container">
            <form action="staf.php" method="GET">
                <input type="text" name="keyword" placeholder="Cari nama, kategori, atau supplier..." value="<?= htmlspecialchars($search); ?>" required>
        ...
        <div class="table-wrapper">
            <table>
                <thead><tr><th>Kode</th><th>Nama Barang</th><th>Kategori</th><th>Pemasok (Supplier)</th><th>Status Stok</th><th>Harga</th></tr></thead>
                <tbody>
                    <?php if(mysqli_num_rows($daftar_barang) > 0): ?>
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="6" style="text-align: center; color: #777;">Data tidak ditemukan.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
